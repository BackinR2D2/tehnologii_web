...
function Note() {
    ...
    const handleSave = async () => {
        const res = await updateNote(id, input, materie, tag, teamMembers);
        if (res.error) {
            toastRef.current.show({severity: 'error', summary: 'Error', detail: res.error});
            return;
        }
            ...
            <div>
                <div style={{display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: '1rem'}}>
                    ...
                    <div className='homepageContainer' style={{padding: '1rem', paddingTop: 0, display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: '1rem'}}>
                      <InputTextarea className='hpTextarea' placeholder='Java - polimorfism' value={input} onChange={handleTextareaChange} style={{resize: 'none', height: 'calc(100vh - 140px)', width: '50%'}} />
